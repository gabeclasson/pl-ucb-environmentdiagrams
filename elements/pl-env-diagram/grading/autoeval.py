...
class FrameTree():
    ...
    def get_bindings(self, codestring, exitlines_pq, exitlines_to_fobj_dict):
        ...
        while len(exitlines_pq) > 0:
            ...
            # TODO: var names
            code_list.insert(i + 4, ' '*whitespace + trk_var_names_dict["frame"] + ".bind(" + trk_var_names_dict["bindings"] + ")")
...
def f():
    ...
    def x(e):
        ...
        return y + e
    list__ = [x(t)]
    list__.extend([10])                # we include tags here as an example, but would recommend against using them. 
    list__.append("Please be careful using this. READ THE COMMENTS ABOVE!")
    return list__
f()"""

ft = FrameTree(testmeowcode)